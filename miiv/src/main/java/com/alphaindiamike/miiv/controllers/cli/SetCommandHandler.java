...
public class SetCommandHandler implements CommandHandler {
	...
	@Autowired
	public SetCommandHandler(GlobalSettingsService settingsService, ConfigurationLoaderService configurationLoader,
	...
	public CommandResponse handle(String[] args) {
		...
		if (args.length == 2) {
			try {
				...
				programState.triggerStateTransition(new WorkspaceReadyState(path.toString()),
	...
	public CommandResponse supports(String command) {
		if (command.equals("init") && (settingsService.getSetting("workspace_dir") != null)) {
